...
module Core (Hash : Hash) (D : Desc) = struct
  ...
  let digest_bigstring ?off ?len buf =
    feed_bigstring empty ?off ?len buf |> get
...
module Make_BLAKE2 (H : Hash_BLAKE2) (D : Desc) = struct
  ...
  module Keyed = struct
    ...
    let maci_bytes ~key iter =
      let ctx =
        H.with_outlen_and_bytes_key digest_size key 0 (By.length key)
      in
    ...
    let maci_bigstring ~key iter =
      let ctx =
        H.with_outlen_and_bigstring_key digest_size key 0 (Bi.length key)
      in
...
let module_of : type k. k hash -> (module S with type kind = k) =
 fun hash ->
  ...
  | BLAKE2B digest_size -> (
    ...
    | exception Not_found ->
        let m : (module S with type kind = [`BLAKE2B]) =
          (module Make_BLAKE2B (struct let digest_size = digest_size
          end)
          : S
            with type kind = [`BLAKE2B] )
        in
  ...
  | BLAKE2S digest_size -> (
    ...
    | exception Not_found ->
        let m =
          (module Make_BLAKE2S (struct let digest_size = digest_size
          end)
          : S
            with type kind = [`BLAKE2S] )
        in
...
let of_digest (type hash kind)
    (module H : S with type t = hash and type kind = kind) (hash : H.t) :
    kind t =
